...
2026-06-24: **Skill audit** — Full audit of 15 skills against Thariq's 9 categories complete. Gaps found: Product Verification (cat 2) and CI/CD & Deployment (cat 7). All descriptions already follow Tip 6 (trigger patterns). `wiki-mode` description trimmed. Report at [[skill-audit-2026-06-24]].

2026-06-24: **Ingest: Thariq Tips — Agent Skills Guide** — Ingested Anthropic's internal guide on how they use Claude Code skills (by Thariq/@trq212, Mar 17 2026). Covers 9 skill categories (Library & API Ref, Product Verification, Data Fetching, Business Automation, Code Scaffolding, Quality & Review, CI/CD, Runbooks, Infrastructure Ops) and 9 crafting principles (gotchas, progressive disclosure, description-for-model, scripts, on-demand hooks, etc.). Created [[Agent Skill Categories]], [[Skill Crafting Principles]], [[Thariq]]. This taxonomy is directly applicable to auditing this vault's 15 skills.

2026-06-23: **Autoresearch: opencode-game-studio and claude-obsidian** — 2 rounds, 3 new sources + 4 existing, 2 new pages. Researched the architectural relationship between OpenCode Game Studios (game dev agents) and claude-obsidian (knowledge vault skills). No direct integration exists — they're independent projects with no cross-references. But they share deep kinship: both implement the Agent Skills spec from kepano/obsidian-skills, both run on OpenCode's free Big Pickle model, and both impose structure on unstructured AI sessions (OCGS for game building, claude-obsidian for knowledge building). The Superpowers plugin is the strongest existing bridge. A combined workflow is feasible: OCGS agents build games while claude-obsidian skills document architecture decisions, design rationale, and playtest results to the vault. Created 2 new pages: [[Research: opencode-game-studio and claude-obsidian]] (synthesis), [[Agent Skills Ecosystem]] (concept). Updated [[OpenCode Game Studios]] and [[OpenCode]] with cross-references.

2026-06-22: **Autoresearch: ChatGPT as planner, OpenCode as executor for game studios** — 2 rounds, 7 sources, 4 new pages + 1 update. Researched the multi-model planner-executor pattern: ChatGPT/GPT-5.5/Opus for game design docs and architecture planning, OpenCode Build agent/Sonnet/DeepSeek for execution, independent reviewer model for quality gates. Found explicit implementations: OCGS tiered model assignment (directors on heavy reasoners), OpenCode Plan Manager (Zod-validated Planner->Builder with filesystem Kanban), betterspec (Planner+Builder+Validator+Archivist agents), SDLC Wizard recipes (Opus->GPT-5.5 review->Sonnet builder->GPT-5.5 review with >=95% confidence gates). Cost optimization ~5x vs single-model. Key insight: this pairs naturally with OCGS — configure director agents to use ChatGPT/GPT for planning, specialists for Big Pickle execution.

2026-06-22: **Autoresearch: game development with opencode** — 2 rounds, 5 sources, 8 pages created. Researched the game dev ecosystem around OpenCode: the dominant pattern is "game studio in a config" via OpenCode Game Studios (54 agents, 37 skills, free on Big Pickle). Also found game-creator (Phaser/Three.js browser games in ~10 min), a real-world case of a Snake game built with zero manual code + local LLM, and multiple community forks (striderZA, oshinono, Diegiwg). Key insight: OpenCode's subagent architecture maps naturally to game studio roles (directors, leads, specialists), analogous to how claude-obsidian structures knowledge work. Open questions: Big Pickle vs paid-model performance for complex game logic, real commercial-quality evidence, optimal agent count per project.
